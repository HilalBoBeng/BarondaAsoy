...
const sendAdminWelcomeEmail = async (email: string, name: string, accessCode: string, role: 'admin' | 'bendahara' | 'petugas') => {
  ...
  const mailOptions = {
    ...
    to: email,
    subject: `Selamat! Akun ${roleName} Baronda Anda Telah Dibuat`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #ddd; border-radius: 10px; overflow: hidden;">
        <div style="background-color: #6f42c1; color: white; padding: 20px; text-align: center;">
          ...
          <h1 style="margin: 0; font-size: 24px;">Akun ${roleName} Dibuat</h1>
        </div>
        <div style="padding: 30px; text-align: center; color: #333;">
          <p style="font-size: 16px;">Halo ${name}, selamat! Akun Anda sebagai ${roleName} Baronda telah berhasil dibuat.</p>
          <p style="font-size: 16px;">Berikut adalah kode akses rahasia Anda. Gunakan kode ini untuk masuk ke dasbor. Jangan bagikan kode ini kepada siapapun.</p>
...
const createAdminFlow = ai.defineFlow(
  ...
  async ({ name, email, phone, addressType, addressDetail, role }) => {
    try {
      ...
      await sendAdminWelcomeEmail(email, name, accessCode, role);
