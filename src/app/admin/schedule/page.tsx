...
import { PlusCircle, Edit, Trash, Loader2, Calendar as CalendarIcon } from 'lucide-react';
...
const scheduleSchema = z.object({
  ...
  time: z.string().min(1, "Waktu patroli tidak boleh kosong."),
  ...
  const onSubmit = async (values: ScheduleFormValues) => {
    ...
    try {
      if (currentSchedule) {
        ...
        await addDoc(collection(db, 'schedules'), values);
              ...
              {loading ? (
                ...
                schedule.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell>{item.date instanceof Date ? item.date.toLocaleDateString('id-ID') : 'N/A'}</TableCell>
                    <TableCell>{item.time}</TableCell>
                    <TableCell>{item.officer}</TableCell>
                    <TableCell>{item.area}</TableCell>
                    <TableCell><Badge variant={item.status === 'Completed' ? 'secondary' : item.status === 'In Progress' ? 'default' : 'outline'}>{item.status}</Badge></TableCell>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormField control={form.control} name="time" render={({ field }) => (
                  <FormItem><FormLabel>Waktu (e.g., 20:00 - 22:00)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )} />
